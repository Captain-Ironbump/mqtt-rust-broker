...
async fn main() -> std::io::Result<()> {
    let dispatcher: MqttPacketDispatcher = MqttPacketDispatcher::new().expect("Failed to create dispatcher");   
    ...
    while let Ok((stream, _)) = listener.accept().await {
        ...
        spawn(async move {
            if let Ok(ws_stream) = accept_async(stream).await {
                connection_handler(ws_stream).await;
            } else {
...
async fn connection_handler(mut ws_stream: tokio_tungstenite::WebSocketStream<tokio::net::TcpStream>) {
